...
class SignInUseCase(mainScheduler: Scheduler, val accountRepository: AccountRepository) : UseCase<User, User>(mainScheduler) {
    override fun requestWithData(data: User): Flowable<User> {
        return accountRepository.signInByUser(data)
    }
}